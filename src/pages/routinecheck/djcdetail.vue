<template>
  <view class="container" :class="[ISForOld ? 'oldcontainer' : '']">
    <view class="tit"> 待检查 </view>
    <view class="">
      <view class="li"><text>检查名称：</text>{{ info.checkName }}</view>
      <view class="li"><text>关联文保单位：</text>{{ info.pointName }}</view>
      <view class="li"><text>限期时间：</text>{{ info.deadLine.substring(0, 10) }}</view>
      <view class="li"><text>发布时间：</text>{{ info.createTime.substring(0, 10) }}</view>
      <view class="li"><text>分类类别：</text>可用电器</view>
      <view class="li"><text>发布人：</text>{{ format_name(info.jcry) }}</view>
      <view class="li lilast">
        <text>检查事项：</text>
      </view>
      <view class="li gong">
        <view class="num">
          共<text>{{ list.length }}</text
          >项
        </view>
        <view class="">
          已上报<text>{{ ysbLength }}</text>
        </view>
      </view>
      <uni-card class="card" v-for="(item, index) in list" :key="index">
        <view class="num"> 0{{ index + 1 }} </view>
        <view class="text">
          <view class="t1"> 检查类别：{{ item.jclb }} </view>
          <view class=""> 具体检查事项：{{ item.dicDetailName }} </view>
        </view>
        <view class="btn" v-if="item.checkType == null">
          <view class="bt1" @click="clickNewAdd('xc', item)">
            <view class=""> 检查结果上报 </view>
            <text>（已现场处理）</text>
          </view>
        ...
        <view class="btn1" v-else>
          ...
          <uni-icons v-if="item.checkType == 0" type="right" size="22" color="#8E8E93" class="icon"></uni-icons>
          <button type="primary" v-if="item.checkType == 1">已上报<text>（需镇街协助）</text></button>
          <uni-icons v-if="item.checkType == 1" type="right" size="22" color="#8E8E93" class="icon"></uni-icons>
        </view>
      </uni-card>
    </view>
  </view>
  ...
  methods: {
    ...
    async routineCheckInfo(id) {
      ...
      for (let i in this.list) {
        if (this.list[i].checkType) {
          this.ysbLength += 1;
        }
      }
    },
    clickNewAdd(type, item) {
      if (type == 'xc') {
        uni.navigateTo({
          url: `xcsend?item=${JSON.stringify(item)}`,
        });
      } else {
        uni.navigateTo({
          url: 'zjsend?item=' + JSON.stringify(item),
        });
      }
    },
  },
