    ...
    <view class="container" :class="show == 'block' ? '' : 'oldcontainer'">
      <view class="top-box" v-if="false">
        ...
        <view class="process">
          <view class="tap" :style="{ left: `calc(${totalNum}% - 50rpx)` }">
            <view class="text">
              <text
                :style="{ color: totalNum * 1 < 50 ? '#20CF77' : totalNum * 1 < 70 ? '#FFDD37' : totalNum * 1 < 90 ? '#F47828' : '#EA1B1B' }">
                {{ formatZS(totalNum) }}
              </text>
            </view>
            <view class="sign"
              :style="{ background: totalNum * 1 < 50 ? '#20CF77' : totalNum * 1 < 70 ? '#FFDD37' : totalNum * 1 < 90 ? '#F47828' : '#EA1B1B' }">
            </view>
          </view>
        </view>
        <view class="zs">
      ...
      <view class="content">
        <swiper class="image-container" previous-margin="45rpx" next-margin="45rpx" circular autoplay
          :indicator-dots="true" indicator-active-color="#5680FA" indicator-color="#ffffff" @change="swiperChange">
          <swiper-item :class="currentIndex == index ? 'swiper-item' : 'swiper-item-side'"
            v-for="(item, index) in imgList" :key="index" lazy-load @click="banner_redirect"
      ...
      <view class="entrance">
        <view class="item" v-for="(item, index) in entranceList" :key="index" @click="jumpTo(item.to)"
          :style="{ background: `url(${item.imgSrc}) no-repeat center`, 'background-size': '100% 100%' }">
          <text class="text1">
            {{ item.name }}
          </text>
          <text class="text2">
      ...
      <view class="mask" @click="
        () => {
          this.isShowTip = false;
        }
      " v-if="isShowTip">
        <view class="tip-list">
          <text v-for="item in tipPointList" :key="item.id" @click="selectPointName($event, item.publishName)">{{
            item.publishName }}</text>
        </view>
      </view>
    </view>
  </view>
...
export default {
    ...
    return {
      ...
      entranceList: [
        ...
        // },
        {
          ...
          enName: 'equipIndex',
          imgSrc: Img6,
          to: '/pages/inspection/equipIndex',
        },
      ],
      isSearch: false,
      guideList: [],
  ...
  created() {
    ...
    uni.removeStorageSync('getListAndProtecteds');
  },
  //end 埋点02
              ...
              if (trackFlag) {
                switch (trackFlag) {
                  ...
                  case '3':
                    ...
                    break;
                  case '10':
                    uni.setStorageSync('role', '镇街审核人员');
                    this.role = '镇街审核人员';
                    setTimeout(() => {
                      this.analyseRole();
                    }, 0);
      ...
      if (this.role == '文保员') {
        ...
        this.entranceList = [
          ...
          },
          {
            ...
            enName: 'Routine inspection',
            imgSrc: Img2,
            to: '/pages/troubleReportReview/index',
          },
          {
            name: '日常巡查审核',
            enName: 'Training and check',
            imgSrc: Img4,
            to: '/pages/inspectionCheck/index',
          },
          {
            name: '事项处置审核',
            enName: 'Event handling',
            imgSrc: Img3,
            to: '/pages/missionCheck/index',
          },
          {
            name: '文保单位任务',
            enName: 'Document protection file',
