<template>
  <view class="inspectionNewContainer" :class="[ISForOld ? 'oldinspectionNewContainer' : '']">
    <view class="blockCon">
      <view class="formCon">
        <uni-forms :modelValue="formData" :rules="rules" ref="formData">
          ...
          <view class="uni-file-picker uni-mt-5">
            <view class="imageAreaContainer">
              ...
              <view class="width333" @click="popTriggerx" v-if="formData.file && formData.file.length < 5">
                <view class="imgBorderCon">
          ...
          <uni-forms-item label="其他信息：" class="addMarginBottom">
            <view class="textAreaCon">
              <uni-easyinput class="localTextArea otherInfo" type="textarea" v-model="formData.otherDescriptions" :placeholderStyle="placeholderStyle" placeholder="请输入其他信息" maxlength="200" />
              <view class="numberCount">{{ (formData.otherDescriptions && formData.otherDescriptions.length) || 0 }}/200 </view>
            </view>
          </uni-forms-item>
            ...
            <view class="questionItem" v-for="(item, index) in afterPick" :key="index">
              ...
              <view class="radioArea">
                <radio-group @change="(event) => radioChange(event, item)">
                  <label class="radio"> <radio value="11" checked />否 </label>
                  <label class="radio"> <radio value="10" />是 </label>
                </radio-group>
              </view>
              <view class="textArea2Con" v-if="item.isCheck == '10'">
                <uni-easyinput
                  class="localTextArea borderTextArea fakeTextAreaRequired"
                  type="textarea"
                  v-model="item.otherDescriptions"
                  :placeholderStyle="placeholderStyle"
                  placeholder="请输入具体存在问题"
                  maxlength="50"
                />
                <view class="number2Count">{{ (item.otherDescriptions && item.otherDescriptions.length) || 0 }}/50 </view>
              </view>
              <view class="specialFormImgTitle with20Margin">
                <text class="fakeRequired">上传照片：</text>
                <text class="localGreen">最多5张</text>
              </view>
              <view class="uni-file-picker uni-mt-5">
                <view class="imageAreaContainer">
                  ...
                  <view class="width333" @click="popTrigger(index)" v-if="afterPick[index].copyfile && afterPick[index].copyfile.length < 5">
                    <view class="imgBorderCon">
                      <uni-icons type="plusempty" size="60" color="#ccc"></uni-icons>
                    </view>
                  </view>
      ...
      <view class="btnsCon">
        <!-- <button class="cancelBtn" type="default" @click="popAction">添加问题</button> -->
        <button class="confirmBtn" @click="submitForm" type="primary">完成巡检</button>
      </view>
      <uni-popup ref="popup" type="bottom" background-color="#fff" :mask-click="false">
        <view>
          <view class="popupTitle">
            <view class="popupTitleText">选择检查事项</view>
            <uni-icons type="closeempty" size="25" color="#999" class="popupTitleClose" @click="close"></uni-icons>
          </view>
          <view class="uni-list">
            <checkbox-group @change="checkboxChange">
              <label class="uni-list-cell" v-for="item in groupItems" :key="item.value">
                <view class="poplistText">{{ item.name }}</view>
                <view>
                  <checkbox :value="item.value" :checked="item.checked" :class="[item.checked ? 'checkBoxActive' : '']" />
                </view>
              </label>
            </checkbox-group>
          </view>
          <view class="popupCloseCon">
            <view class="addBtn" @click="addAction">确认选择</view>
          </view>
        </view>
      </uni-popup>
    </view>
...
<script>
import { uploadNew, uploadWithDtDream, basecheckCreate, getMyBumpPointBaseInfo, getPhoneToPointDistance } from '@/api/api.js';
...
export default {
  ...
  methods: {
          ...
          uploadWithDtDream({
            ...
            if (res) {
              ...
              if (real && real.code && real.code == 200) {
                ...
                this.afterPick[index].file.push({ url: innerPath, name: innerId, id: innerId });
                this.afterPick[index].copyfile.push({ url: innerPath, name: innerId, id: innerId });
              } else {
                uni.showToast({
                  title: '上传失败',
                  icon: 'none',
          ...
          uploadWithDtDream({
            ...
            if (res) {
              ...
              if (real && real.code && real.code == 200) {
                ...
                this.formData.copyfile.push({ url: innerPath, name: innerId, id: innerId });
              } else {
                uni.showToast({
                  title: '上传失败',
                  icon: 'none',
    ...
    removeCurrentImg(index, jndex) {
      this.afterPick[index].copyfile.splice(jndex, 1);
    },
    ...
    getLocation() {
      ZWJSBridge.getLocation()
        .then((result) => {
          ...
          uni.setStorageSync('InspectionMapData', this.locationObj);
    ...
    coutDownAction() {
      clearInterval(this.timer);
      this.timer = null;
      let start = dayjs(this.formData.startTime);
      this.timer = setInterval(() => {
        let end = dayjs();
        let diff = end.diff(start);
        let duration = dayjs.duration(diff);
        let time = `${String(duration.hours()).length == 1 ? '0' + duration.hours() : duration.hours()}:${String(duration.minutes()).length == 1 ? '0' + duration.minutes() : duration.minutes()}:${
    ...
    analyseStorage() {
      ...
      if (this.ZLBAPIAble) {
        ...
        value1 = uni.getStorageSync('InspectionMapData');
      }
    ...
    submitForm() {
      ...
      const img2Generator = (ele) => {
        if (ele.copyfile && ele.copyfile.length) {
          let middle = ele.copyfile.map((ele) => {
            return {
              id: ele.id,
              img: ele.url,
            };
      ...
      //   });
      // }
      if (!this.formData.otherDescriptions) {
        return uni.showToast({
          title: '请检查输入项',
          icon: 'error',
        });
      ...
      let fixArray1 = [
        {
          ...
          checkMsg: '',
          isCheck: '',
          otherDescriptions: this.formData.otherDescriptions,
          imagesId: img3Generator(),
        },
      ];
      let afterParsePick = this.afterPick.map((ele) => {
        return {
      ...
      afterParsePick = afterParsePick.map((ele) => {
        return {
          checkContent: ele.checkContent,
          checkLabel: ele.checkLabel,
          checkMsg: ele.checkMsg,
      ...
      let fifthFilter = afterParsePick.filter((ele) => {
        return ele.isCheck == '10' && !ele.otherDescriptions; //取出所有有问题且没填的
      });
      ...
      let sixthFilter = afterParsePick.filter((ele) => {
        return !ele.imagesId;
      });
      ...
      //   this.addressText = '测试的万能地址';
      //   this.lng = '0.00';
      //   this.lat = '0.00';
      //   if (!this.addressText || this.addressText == '请选择地址') {
      //     return uni.showToast({
      //       title: '请选择定位或您的位置不在文物检查范围之内',
      //       icon: 'error',
      //     });
      //   }
      ...
      let finalData = {
        ...
        checkType: '3',
        checkFeedback: '',
        checkName: trackSelect ? trackSelect.publishName + '巡检上报' : '巡检上报',
        checkcontent: '',
        ...
        checkBody: [...fixArray1, ...afterParsePick],
      };
    ...
    analyseQuery() {
      ...
      // this.groupItems = listData.map((ele) => {
      //   return {
      //     name: ele.problemName,
      //     value: ele.problemNoIndexName,
      //   };
      // });
      // 不用弹框选，直接页面赋值
      this.afterPick = listData.map((ele) => {
        return {
          checkLabel: ele.problemNoIndexName,
          otherDescriptions: '',
          checkMsg: '',
          checkMsgFlag: 1,
          checkContent: '',
    ...
    removeQuestionAction(index) {
      this.afterPick.splice(index, 1);
    },
    addAction() {
      this.$refs.popup.close();
      this.afterPick = this.middleStage.map((ele) => {
        return {
          checkLabel: ele,
          otherDescriptions: '',
          checkMsg: '',
