    ...
    <view class="blockCon">
      <view class="formCon">
        ...
        <uni-forms :modelValue="formData" :rules="rules" ref="formData">
          ...
          <uni-forms-item label="巡查问题" label-position="top" class="questionFormItem"
            v-if="afterPick && afterPick.length">
            <view class="questionItem" v-for="(item, index) in afterPick" :key="index">
              ...
              <uni-file-picker v-if="item.file && item.file.length" fileMediatype="image"
                file-extname="png,jpg,jpeg,gif" :image-styles="imageStyles" mode="grid" :ref="'file2s' + (index + 1)"
                v-model="item.file" limit="5" @success="success" @fail="fail"></uni-file-picker>
            </view>
          </uni-forms-item>
        </uni-forms>
      </view>
      <view class="statusCon">
        <view class="statusTitle">镇街审核状态：</view>
        <view class="statusBlock"
          :class="[jdSubStatus == '立即处置' ? 'colorRed' : jdSubStatus == '无需处置' ? 'colorGreen' : 'colorYellow']">{{ jdSubStatus || '待审核' }}
        </view>
      </view>
      <view class="statusCon">
        <view class="statusTitle">区文物局审核状态：</view>
        <view class="statusBlock"
          :class="[subStatus == '一网统管' ? 'colorRed' : 'colorYellow']">{{ subStatus }}
        </view>
      </view>
      <view class="btnsCon">
        <button class="confirmBtn" @click="submitForm" type="primary">返回</button>
      </view>
    </view>
  </view>
</template>
      ...
      getbaseCheckDataById({
        ...
        .then((res) => {
          if (res && res.code && res.code == 200) {
            ...
            let jdIsRunState = shorter.baseCheck.jdIsRunState;
            if (jdIsRunState) {
              switch (jdIsRunState) {
                case 1:
                  this.jdSubStatus = '立即处置';
                  break;
                case 99:
                  this.jdSubStatus = '无法处置';
                  break;
                case 4:
                  this.jdSubStatus = '无需处置';
                  break;
              }
            }
          }
