<template>
  <view class="inspectionContainer" :class="[ISForOld ? 'oldinspectionContainer' : '']">
    <view class="switchCon">
      <view class="headTag">设备检查记录</view>
    </view>
            ...
            <view class="firstLine">
              ...
              <view class="firstRight">
                <!-- <view class="rightStatus"
                  :class="[item.subStatus == '待审核' ? 'colorYellow' : item.subStatus == '合格' ? 'colorGreen' : 'colorRed']">
                  {{ item.subStatus }}</view> -->
              </view>
            </view>
            ...
            <view class="fourthLine">
              <template v-for="(sub, subIndex) in item.subArray" :key="subIndex">
                <view class="innerCon flexStart" :class="[subIndex == 0 || subIndex == 2 ? '' : '']">
                  ...
                  >：
                  <view class="innerContent" :class="[sub.content == '异常' || sub.content == '有隐患' ? 'unUsual' : '']"> {{ sub.content }}</view>
                </view>
              </template>
            </view>
          </view>
    ...
    <view class="fixBottom fixPaddingBottom">
      <view class="addBtn" @click="addAction">设备检查上报</view>
    </view>
  </view>
</template>
<script>
...
export default {
  ...
  methods: {
              ...
              shorter.list.forEach((ele) => {
                ...
                if (ele.baseCheckDetailList) {
                  ...
                  let track4 = ele.baseCheckDetailList.find((sub) => {
                    return sub.checkNameType == '其他';
                  });
                  /* if (
                    (track1 && track1.dicDetailName != '正确') ||
                    (track2 && track2.dicDetailName != '正确') ||
                    (track3 && track3.dicDetailName != '正确') ||
                    (track4 && track4.dicDetailName != '正常')
                  ) {
                    ele.subArray[1] = { title: '设备状态', content: '异常' };
                  } else {
                    ele.subArray[1] = { title: '设备状态', content: '正常' };
                  } */
                  if ((track1 && !track1.isCheck) || (track2 && !track2.isCheck) || (track3 && !track3.isCheck) || (track4 && track4.otherDescriptions)) {
                    ele.subArray[1] = { title: '设备状态', content: '异常' };
                  } else {
                    ele.subArray[1] = { title: '设备状态', content: '正常' };
                  }
                  if ((track1 && track1.checkContent) || (track2 && track2.checkContent) || (track3 && track3.checkContent)) {
                    ele.subArray[0] = { title: '设备数量', content: '异常' };
                  } else {
                    ele.subArray[0] = { title: '设备数量', content: '正常' };
                  }
                }
              });
              this.total = shorter.total;
    ...
    addAction() {
      ...
      uni.navigateTo({
        url: '/pages/inspection/equipNew',
      });
    },
    toDetail(item) {
      uni.navigateTo({
        url: '/pages/inspection/equipDetail' + '?id=' + item.baseCheck.id,
      });
    },
  },
};
